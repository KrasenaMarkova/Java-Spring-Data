package bg.softuni._16_softunigamestore.controller;

import bg.softuni._16_softunigamestore.dto.*;
import bg.softuni._16_softunigamestore.service.GameService;
import bg.softuni._16_softunigamestore.service.UserService;
...
public class CommandLineRunner implements org.springframework.boot.CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        while (!(line = line = reader.readLine()).equals("stop")) {
            ...
            switch (tokens[0]) {
                ...
                case "LoginUser":
                    ...
                    break;
                case "Loginout":
                    command = this.userService.loginout();
                    break;
                case "AddGame":
                ...
                case "AllGames":
                    ...
                    break;
                case "DetailGame":
                    command = String.valueOf(this.gameService.getDetailGame(tokens[1]));
                    break;
                case "OwnedGames":
                    command = this.gameService.getOwnedGame();
                    break;
                case "AddItem":
                    command = this.gameService.addItem(tokens[1]);
                    break;
                case "RemoveItem":
                    command = this.gameService.removeItem(tokens[1]);
                    break;
                case "BuyItem":
                    command = this.gameService.buyItem(tokens[1]);
                    break;
            }
